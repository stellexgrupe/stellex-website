...
export const ReadyPostsDepartment: React.FC<ReadyPostsDepartmentProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<'prefab' | 'starter' | 'artistic_handrail'>('prefab');
  ...
  const prefabCount = READY_POST_MODELS.filter((p) => p.postCategory === 'prefab').length;
  const starterCount = READY_POST_MODELS.filter((p) => p.postCategory === 'starter').length;
  const artisticHandrailCount = READY_POST_MODELS.filter((p) => p.postCategory === 'artistic_handrail').length;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="space-y-3">
              ...
              {/* Compact Sub-Menu Filter Tabs (3 Required Categories) */}
              <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-slate-950/80 border border-slate-800 text-xs">
                <button
                  onClick={() => {
                    setSelectedCategory('prefab');
                    if (selectedPost && selectedPost.postCategory !== 'prefab') {
                      setSelectedPostId(null);
                    }
                  }}
                  className={`py-2 px-1.5 rounded-lg font-bold transition-all cursor-pointer text-center text-[11px] sm:text-xs flex items-center justify-center gap-1.5 truncate ${
                    selectedCategory === 'prefab'
                      ? 'bg-amber-500 text-slate-950 shadow font-black'
                      : 'text-slate-300 hover:text-white hover:bg-slate-900'
                  }`}
                >
                  <Layers className="w-3.5 h-3.5 shrink-0" />
                  <span className="truncate">{language === 'fa' ? `پایه‌های پیش‌ساخته (${prefabCount})` : `Prefab Posts (${prefabCount})`}</span>
                </button>

                <button
                  onClick={() => {
                    setSelectedCategory('starter');
                    if (selectedPost && selectedPost.postCategory !== 'starter') {
                      setSelectedPostId(null);
                    }
                  }}
                  className={`py-2 px-1.5 rounded-lg font-bold transition-all cursor-pointer text-center text-[11px] sm:text-xs flex items-center justify-center gap-1.5 truncate ${
                ...
                >
                  ...
                  <span className="truncate">{language === 'fa' ? `استارترهای لوکس (${starterCount})` : `Luxury Starters (${starterCount})`}</span>
                </button>

                <button
                  onClick={() => {
                    setSelectedCategory('artistic_handrail');
                    if (selectedPost && selectedPost.postCategory !== 'artistic_handrail') {
                      setSelectedPostId(null);
                    }
                  }}
                  className={`py-2 px-1.5 rounded-lg font-bold transition-all cursor-pointer text-center text-[11px] sm:text-xs flex items-center justify-center gap-1.5 truncate ${
                    selectedCategory === 'artistic_handrail'
                      ? 'bg-amber-500 text-slate-950 shadow font-black'
                      : 'text-slate-300 hover:text-white hover:bg-slate-900'
                  }`}
                >
                  <Palette className="w-3.5 h-3.5 shrink-0" />
                  <span className="truncate">{language === 'fa' ? `هندریل‌های هنری (${artisticHandrailCount})` : `Artistic Handrails (${artisticHandrailCount})`}</span>
                </button>
              </div>
            </div>
                  ...
                  >
                    ...
                    {post.postCategory === 'prefab' && (
                      <span className="absolute top-1.5 right-1.5 rtl:right-1.5 rtl:left-auto bg-slate-950/85 border border-amber-500/40 text-amber-400 text-[9px] font-black px-1.5 py-0.5 rounded shadow backdrop-blur-md flex items-center gap-0.5 z-10">
                        ...
                        <span>{language === 'fa' ? 'پیش‌ساخته' : 'Prefab'}</span>
                      </span>
                    )}
                    {post.postCategory === 'starter' && (
                      <span className="absolute top-1.5 right-1.5 rtl:right-1.5 rtl:left-auto bg-slate-950/85 border border-amber-500/40 text-amber-400 text-[9px] font-black px-1.5 py-0.5 rounded shadow backdrop-blur-md flex items-center gap-0.5 z-10">
                        <Sparkles className="w-2.5 h-2.5 text-amber-400" />
                        <span>{language === 'fa' ? 'استارتر لوکس' : 'Starter'}</span>
                      </span>
                    )}
                    {post.postCategory === 'artistic_handrail' && (
                      <span className="absolute top-1.5 right-1.5 rtl:right-1.5 rtl:left-auto bg-slate-950/85 border border-amber-500/40 text-amber-400 text-[9px] font-black px-1.5 py-0.5 rounded shadow backdrop-blur-md flex items-center gap-0.5 z-10">
                        <Palette className="w-2.5 h-2.5 text-amber-400" />
                        <span>{language === 'fa' ? 'هندریل هنری' : 'Handrail'}</span>
                      </span>
                    )}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-xs font-black text-amber-300 font-mono">
                    {language === 'fa' ? '۱۰ الی ۱۵ سال ضمانت کتبی تعویض' : '10 to 15 Years Written Warranty'}
                  </div>
                </div>
              </div>
              ...
              <div className="lux-card rounded-3xl overflow-hidden border border-amber-500/40 hover:border-amber-400/80 shadow-2xl transition-all duration-300 flex flex-col bg-slate-950/90 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="relative min-h-[340px] sm:min-h-[400px] max-h-[460px] w-full overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4 group border-b border-slate-800">
                  ...
                  <div className="absolute top-3 inset-x-3 flex items-center justify-between z-20 gap-2 pointer-events-none">
                    ...
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      {selectedPost.postCategory === 'prefab' && (
                        <span className="text-[10px] font-black bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 px-2.5 py-1 rounded-full shadow-md shrink-0 flex items-center gap-1">
                          ...
                          <span>{language === 'fa' ? 'پایه پیش‌ساخته' : language === 'ar' ? 'قائمة جاهزة' : 'Pre-Fabricated'}</span>
                        </span>
                      )}
                      {selectedPost.postCategory === 'starter' && (
                        <span className="text-[10px] font-black bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 px-2.5 py-1 rounded-full shadow-md shrink-0 flex items-center gap-1">
                          <Sparkles className="w-3 h-3" />
                          <span>{language === 'fa' ? 'استارتر لوکس' : language === 'ar' ? 'عامود بداية فاخر' : 'Luxury Starter'}</span>
                        </span>
                      )}
                      {selectedPost.postCategory === 'artistic_handrail' && (
                        <span className="text-[10px] font-black bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 px-2.5 py-1 rounded-full shadow-md shrink-0 flex items-center gap-1">
                          <Palette className="w-3 h-3" />
                          <span>{language === 'fa' ? 'هندریل هنری' : language === 'ar' ? 'هندريل فني' : 'Artistic Handrail'}</span>
                        </span>
                      )}
                ...
                <div className="p-5 sm:p-6 space-y-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Category-Specific Pricing Presentation */}
                    {selectedUnitPrice && selectedLinearPrice && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        
                        {/* Unit Price per post (For Prefab and Starter) */}
                        {selectedPost.postCategory !== 'artistic_handrail' && (
                          <div className={`p-3.5 rounded-2xl bg-slate-950/70 border border-slate-800/80 flex flex-col justify-between ${
                            selectedPost.postCategory === 'starter' ? 'sm:col-span-2' : ''
                          }`}>
                            <span className="text-[11px] text-slate-400 font-medium">
                              {selectedPost.postCategory === 'starter'
                                ? (language === 'fa' ? 'قیمت هر عدد استارتر لوکس:' : language === 'ar' ? 'سعر عامود البداية:' : 'Starter Unit Price:')
                                : (language === 'fa' ? 'قیمت هر عدد پایه پیش‌ساخته:' : language === 'ar' ? 'سعر القائمة الجاهزة:' : 'Unit Post Price:')}
                            </span>
                        ...
                        {/* Linear meter installed price with all materials (For Handrail and Prefab) */}
                        {selectedPost.postCategory !== 'starter' && (
                          <div className={`p-3.5 rounded-2xl bg-gradient-to-br from-emerald-950/30 to-slate-950 border border-emerald-500/30 flex flex-col justify-between ${
                            selectedPost.postCategory === 'artistic_handrail' ? 'sm:col-span-2' : ''
                          }`}>
                            <span className="text-[11px] text-emerald-400 font-bold">
                              {selectedPost.postCategory === 'artistic_handrail'
                                ? (language === 'fa' ? 'قیمت هر متر طول هندریل هنری (با نصب و متعلقات):' : language === 'ar' ? 'سعر المتر الطولي للدرابزين مع التركيب:' : 'Handrail per Linear Meter:')
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold">
                      ...
                      <span>
                        {language === 'fa' 
                          ? 'دپارتمان تخصصی پایه‌های آماده پیش‌ساخته استیلکس' 
                          : language === 'ar' 
                          ? 'قسم القوائم الجاهزة المعمارية - ستيليكس' 
                          : 'STELLEX Architectural Modular Ready Posts Department'}
                      </span>
                    </div>
                    ...
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold">
                      ...
                      <span>
                        {language === 'fa' 
                          ? 'قیمت هر عدد پایه: از ۱.۸۵ تا ۴.۶ میلیون تومان' 
                          : language === 'ar' 
                          ? `سعر القائم: من ${minPriceConverted.formattedNumber} إلى ${maxPriceConverted.formattedNumber}` 
                          : `Unit Price: $${minPriceConverted.formattedNumber} - $${maxPriceConverted.formattedNumber} USD`}
                      </span>
                    </div>
                  </div>
                  ...
                  <h3 className="text-xl sm:text-2xl lg:text-3xl font-black text-white tracking-tight">
                    {language === 'fa'
                      ? 'تولید صنعتی پایه‌های پیش‌ساخته آماده نصب فوری با قطعات CNC و اتصالات فابریک'
                      : language === 'ar'
                      ? 'تصنيع صناعي للقوائم الجاهزة للتركيب الفوري مع قطع CNC وملحقات متكاملة'
                      : 'Industrial Prefabricated Ready Posts with CNC Precision & Modular Hardware'}
                  </h3>

                  <p className="text-sm text-slate-300 leading-relaxed">
                    {language === 'fa'
                      ? 'حذف کامل گرد و غبار، حرارت و سیاهی جوشکاری در محل ساختمان با بهره‌گیری از پایه‌های مهندسی‌شده استیلکس. این پایه‌ها در ارتفاع‌های استاندارد ۸۵ الی ۱۱۰ سانتی‌متر با ماشین‌کاری CNC و پرداخت سوپرمیرور یا پوشش نانو PVD تیتانیوم تولید شده و به همراه قالپاق، انکر بولت و متعلقات کامل فابریک تحویل داده می‌شوند.'
                      : language === 'ar'
                      ? 'إلغاء أعمال اللحام والدخان في موقع البناء مع قوائم ستيليكس الجاهزة والمصنعة بدقة CNC وطلاء PVD مفرغ والمرفقة بكامل الإكسسوارات والملحقات الفاخرة.'
                      : 'Eliminate jobsite welding smoke, burn marks, and cleanup with STELLEX precision modular ready posts. Produced with 5-axis CNC machining, mirror finishes, and high-vacuum PVD titanium coating, delivered as turnkey packages.'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/40 transition-colors">
                    <div className="flex items-center gap-2 mb-1.5 text-amber-400">
                      ...
                      <h5 className="text-xs font-black text-slate-100">
                        {language === 'fa' ? 'نصب مکانیکی تمام‌پیچ و مهره' : '100% Mechanical Bolt-on'}
                      </h5>
                    </div>
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      {language === 'fa'
                        ? 'مونتاژ بسیار سریع با آلن و انکربولت بدون نیاز به کپسول گاز، ژنراتور و سنگ فرز.'
                        : 'Rapid clean assembly with Hex keys and anchors without welding generators.'}
                    </p>
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/40 transition-colors">
                    <div className="flex items-center gap-2 mb-1.5 text-amber-400">
                      <CheckCircle2 className="w-4 h-4 shrink-0" />
                      <h5 className="text-xs font-black text-slate-100">
                        {language === 'fa' ? 'ماشین‌کاری CNC و دقت ابعادی' : 'CNC Machining Precision'}
                      </h5>
                    </div>
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      {language === 'fa'
                        ? 'سوراخ‌کاری و قلاویز فابریک کارخانه‌ای با تلرانس کمتر از ۰.۰۵ میلی‌متر و بیس‌پلیت باربر ضخیم.'
                        : 'Factory CNC tapping with tolerance < 0.05mm and heavy load-bearing base plates.'}
                    </p>
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/40 transition-colors">
                    <div className="flex items-center gap-2 mb-1.5 text-amber-400">
                      <CheckCircle2 className="w-4 h-4 shrink-0" />
                      <h5 className="text-xs font-black text-slate-100">
                        {language === 'fa' ? 'آلیاژهای ۳۰۴/۳۱۶ نگیر و فینیشینگ PVD' : '304/316 Alloys & Vacuum PVD'}
                      </h5>
                    </div>
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      {language === 'fa'
                        ? 'تضمین مادام‌العمر ضد زنگ با پوشش نانو PVD ضدخش در رنگ‌های طلایی، مشکی دودی، نقره‌ای و رزگلد.'
                        : 'Lifetime anti-rust guarantee with scratch-resistant PVD coating in Gold, Black, Silver & Rose Gold.'}
                    </p>
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/40 transition-colors">
                    <div className="flex items-center gap-2 mb-1.5 text-amber-400">
                      <CheckCircle2 className="w-4 h-4 shrink-0" />
                      <h5 className="text-xs font-black text-slate-100">
                        {language === 'fa' ? 'پکینگ صادراتی و پک کامل یراق‌آلات' : 'Export Packaging & Complete Kit'}
                      </h5>
                    </div>
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      {language === 'fa'
                        ? 'بسته‌بندی در فوم ضربه‌گیر به همراه قالپاق پرسی، براکت دست‌انداز و انکربولت بدون نیاز به قطعه مجزا.'
                        : 'Protective foam packaging including flange covers, brackets, and anchor bolts.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-500/15 via-slate-900 to-amber-500/10 border border-amber-500/30 flex items-center gap-3">
                  ...
                  <div className="text-xs text-slate-300 leading-relaxed">
                    ...
                    {language === 'fa'
                      ? 'روی هر یک از پایه‌ها در گالری روبرو کلیک فرمایید تا کارت مشخصات فنی، تصاویر زوم و استعلام آنلاین برای آن مدل در این بخش نمایان شود.'
                      : 'Click on any post in the gallery to reveal its full technical specifications, pricing, zoom photos, and instant consultation here.'}
                  </div>
                </div>
                ...
                <div className="pt-2 flex flex-wrap items-center gap-3">
                  <button
                    onClick={() => onOpenConsultation('مشاوره تخصصی پایه‌های آماده استیلکس')}
                  ...
                  >
                    ...
                    <span>
                      {language === 'fa' 
                        ? 'مشاوره و استعلام خرید تیراژ' 
                        : 'Consultation & Bulk Quoting'}
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span>
                      {language === 'fa' ? 'محاسبه‌گر پلاس مهندسی' : 'Engineering Calculator Plus'}
                    </span>
                  </button>
                </div>
